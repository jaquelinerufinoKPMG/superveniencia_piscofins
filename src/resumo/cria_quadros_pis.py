...
COSIF_QUADRO_2 = CONTA_QUADRO_2
COSIF_QUADRO_2_DESCRICAO = "SUPERVENIÊNCIA DE DEPRECIAÇÃO"
...
QUADRO_2_COLUMNS = ["Conta", "Valor Líquido", "Num Contrato", "Ano"]
QUADRO_3_COLUMNS = ["Conta", "COSIF", "Num Contrato", "Ano", "Valor Líquido"]
ContratoFiltro = str | int | Iterable[str | int]
...
def _remover_linha_inicial_field(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df

    primeiro_valor = str(df.iloc[0, 0]).strip().lower()
    if primeiro_valor.startswith("field"):
...
def _normalizar_contratos_filtro(contrato: ContratoFiltro | None) -> set[str] | None:
    if contrato is None:
        return None

    valores = [contrato] if isinstance(contrato, (str, int)) else list(contrato)
    contratos = {str(valor).strip() for valor in valores if pd.notna(valor)}
    contratos = {valor for valor in contratos if valor}

    for valor in list(contratos):
        if valor.isdigit():
            contratos.add(str(int(valor)))
            contratos.add(valor.zfill(7))

    return contratos or None


def _filtrar_por_contrato(
    df: pd.DataFrame, contrato: ContratoFiltro | None
) -> pd.DataFrame:
    contratos = _normalizar_contratos_filtro(contrato)
    if not contratos:
        return df

    return df[df["Num Contrato"].astype("string").str.strip().isin(contratos)].copy()


def _ler_conta_grafica_colunas(
    conta_grafica: pd.DataFrame | str | Path,
    columns: list[str],
    contrato: ContratoFiltro | None = None,
) -> pd.DataFrame:
    if isinstance(conta_grafica, pd.DataFrame):
        return _filtrar_por_contrato(conta_grafica[columns].copy(), contrato)

    conta_grafica_path = Path(conta_grafica)
    if conta_grafica_path.suffix.lower() == ".parquet":
        filtros = _normalizar_contratos_filtro(contrato)
        read_kwargs = {"columns": columns}
        if filtros:
            read_kwargs["filters"] = [("Num Contrato", "in", sorted(filtros))]
        df = pd.read_parquet(conta_grafica_path, **read_kwargs)
        return _filtrar_por_contrato(df, contrato)

    if conta_grafica_path.suffix.lower() == ".csv":
        df = pd.read_csv(
            conta_grafica_path,
            sep=";",
            encoding="utf-8",
    ...
    raise ValueError(
        "Formato de conta gráfica não suportado. Use DataFrame, .parquet ou .csv."
    )
...
def preparar_conta_grafica(
    ...
    df = _remover_linha_inicial_field(df)
...
def cria_quadro_1(
    ...
    conta_grafica_df = _ler_conta_grafica_colunas(
        conta_grafica, QUADRO_1_COLUMNS, contrato
    )
    resultados = []

    for cosif, descricao in COSIFS_QUADRO_1.items():
        recorte_df = (
            conta_grafica_df[conta_grafica_df["COSIF - Filtro"] == cosif]
...
def cria_quadro_2(
    ...
    conta_grafica_df = _ler_conta_grafica_colunas(
        conta_grafica, QUADRO_2_COLUMNS, contrato
    )
    quadro_2 = conta_grafica_df[conta_grafica_df["Conta"] == CONTA_QUADRO_2].copy()
...
def cria_quadro_3(
    ...
    conta_grafica_df = _ler_conta_grafica_colunas(
        conta_grafica, QUADRO_3_COLUMNS, contrato
    )
    quadro_3 = conta_grafica_df[conta_grafica_df["Conta"].isin(contas_filtro)].copy()
